...
export async function createService(config) {
  ...
  await app.register(fastifyStatic, {
    ...
    immutable: false,
    dotfiles: 'deny',
    /* index.html is a template rendered per request, never served raw. Left
       reachable it is a second copy of the home page with `%TITLE%` for a
       title, no canonical link and nothing filled in — and one a crawler will
       happily index alongside the real thing. */
  ...
  function sendHtml(reply, req, { target } = {}) {
    ...
    return reply
      ...
      /* Belt and braces with the meta tag above: a header covers the case
         where the same URL is fetched as data, which is a crawlable URL too. */
      .header('x-robots-tag', label ? 'noindex, follow' : 'index, follow')
      .send(html);
  }
  ...
  app.get('/api', { config: scanLimit }, async (req, reply) => {
    ...
    // A data route refuses a format it does not know rather than quietly
    // picking one — including this one, now that it can return a report.
    if (wanted === 'invalid') return badOutput(reply, pickLang(req));
    const format = wanted === 'yaml' ? 'yaml' : 'json';
    if (homeTarget) return serveScan(req, reply, homeTarget(req), format);
    return sendData(reply, format, USAGE);
  });
